    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
        ...
        <div class="p-4 flex flex-col h-[180px]">
            ...
            <div class="mt-4 flex items-center gap-2">
                <button class="w-full flex items-center justify-center gap-1 py-2 bg-indigo-600 text-white text-xs font-bold rounded-xl transition-all active:scale-95 shadow-sm cursor-pointer">
                    Pinjam Sekarang
                </button>
                @if($item->stock > 0)
                <button @click="$store.cart.add(@js($item))" class="w-max flex items-center justify-center gap-1 px-2 py-2 bg-white border border-indigo-600 text-indigo-600 hover:bg-indigo-600 hover:text-white text-xs font-bold rounded-xl transition-all active:scale-95 shadow-sm cursor-pointer">
                    <x-heroicon-o-shopping-cart class="w-4 h-4 font-semibold" />
                </button>
                @else
                <button disabled class="w-full py-2 bg-gray-50 border border-gray-200 text-gray-400 text-xs font-bold rounded-xl cursor-not-allowed">
                    Tidak Tersedia
                </button>
                @endif
            </div>
